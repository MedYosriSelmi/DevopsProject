...
public class TimesheetServiceImpl implements ITimesheetService {
	...
	public void affecterMissionADepartement(int missionId, int depId) {
		...
		Optional<Mission> miOptional = missionRepository.findById(missionId);
		Optional<Departement> deOptional = deptRepoistory.findById(depId);

		if (miOptional.isPresent() && deOptional.isPresent()) {

			Mission mission = miOptional.get();

			mission.setDepartement(deOptional.get());
			missionRepository.save(mission);
		}
	}
	...
	public void validerTimesheet(int missionId, int employeId, Date dateDebut, Date dateFin, int validateurId) {
		...
		Optional<Mission> misOptional = missionRepository.findById(missionId);

		if (emOptional.isPresent() && misOptional.isPresent()) {

			Employe validateur = emOptional.get();
			Mission mission = misOptional.get();
